...
| [`offline-bundle/`](offline-bundle/) | A fully offline, no-internet distributable of the app — same functionality as `index.html`, but with all JS libraries vendored locally. Includes double-click launchers (`start.sh` / `start.bat`) and its own setup instructions. Download this if you need to run the tool with no internet connection. |
